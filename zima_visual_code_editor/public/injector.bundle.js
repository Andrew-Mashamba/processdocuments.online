...
(function () {
    ...
    let allChanges = []; // Combined: AI commands + visual edits
    let elementActionTooltip = null;
    let contextElements = []; // Elements added to context
    // Import DOM executor
    const domExecutor = {
        changeLog: [],
        ...
        setText(selector, text, type = 'innerText') {
            const elements = document.querySelectorAll(selector);
            elements.forEach((el) => {
                if (type === 'innerHTML')
                    el.innerHTML = text;
        ...
        appendChild(parentSelector, childHtml) {
            const parent = document.querySelector(parentSelector);
            if (!parent)
                throw new Error(`Parent not found: ${parentSelector}`);
        ...
        wrapText(selector, text, tag, classes, styles) {
            const element = document.querySelector(selector);
            if (!element)
                throw new Error(`Element not found: ${selector}`);
        ...
        // ==========================================
        loadGoogleFont(fontFamily, weights, styles) {
        ...
        loadCustomFont(fontFamily, fontUrl, fontWeight, fontStyle, format) {
      ...
      `;
        ...
        getAvailableFonts() {
            ...
            document.querySelectorAll('link[href*="fonts.googleapis.com"]').forEach((link) => {
                ...
                if (match)
                    loadedFonts.push(match[1].replace(/\+/g, ' '));
            });
        ...
        setFontFallback(selector, fontStack) {
            const elements = document.querySelectorAll(selector);
        ...
        // ==========================================
        enableDragging(selector, axis = 'both', containment) {
            const elements = document.querySelectorAll(selector);
            elements.forEach((el) => {
                    ...
                    if (containment) {
                        ...
                        if (container) {
                            ...
                            if (elRect.right > containerRect.right)
                                el.style.left = `${containerRect.width - elRect.width}px`;
                            if (elRect.bottom > containerRect.bottom)
                                el.style.top = `${containerRect.height - elRect.height}px`;
                        }
                    }
                };
                const onMouseUp = () => {
        ...
        setPosition(selector, position, top, left, right, bottom, zIndex) {
            const elements = document.querySelectorAll(selector);
            elements.forEach((el) => {
        ...
        moveToCoordinates(selector, x, y, transition = false) {
            const elements = document.querySelectorAll(selector);
            elements.forEach((el) => {
                if (getComputedStyle(el).position === 'static') {
        ...
        swapElements(selector1, selector2, animate = false) {
        ...
        reorderChildren(parentSelector, order) {
            const parent = document.querySelector(parentSelector);
            if (!parent)
                throw new Error(`Parent not found: ${parentSelector}`);
    ...
    async function executeDOMTool(toolName, params) {
    ...
    function enableElementSelection() {
        ...
        document.addEventListener('click', async (e) => {
            ...
            if (commandPanel && commandPanel.contains(target) ||
                target.id === 'zima-visual-editor-toggle' ||
                target.closest('#zima-visual-editor-toggle') ||
                target.closest('#zima-command-panel') ||
                target.id === 'visual-editor-overlay' ||
    ...
    async function captureElementContext(element) {
        const rect = element.getBoundingClientRect();
        return {
            visual: {
    ...
    function detectComponentBinding(element) {
        // Livewire
    ...
    `;
        ...
        document.getElementById('submit-command-btn').onclick = async () => {
            ...
            const contexts = await Promise.all(selectedElements.map(el => captureElementContext(el)));
            if (ws) {
                ws.send(JSON.stringify({
    ...
    window.removeSelectedElement = (index) => {
    ...
    // ==========================================
    function showElementActionTooltip(element, clickX, clickY) {
    ...
    function closeTooltipOnOutsideClick(e) {
        if (elementActionTooltip && !elementActionTooltip.contains(e.target)) {
            closeTooltip();
        }
    }
    ...
    function handleTooltipAction(action, element) {
        closeTooltip();
        switch (action) {
            case 'add-to-context':
    ...
    function addElementToContext(element) {
    ...
    function enableElementDragging(element) {
        // Make element draggable
    ...
    `;
        ...
        document.getElementById('font-load').onclick = () => {
            const preset = document.getElementById('font-preset').value;
            const custom = document.getElementById('font-custom').value;
    ...
    function updateStylePanelFontList(newFont) {
        const fontSelect = document.getElementById('vis-font-family');
        if (fontSelect) {
    ...
    let componentPreviewModal = null;
    let currentComponentHtml = '';
    function showComponentInserter(targetElement) {
    ...
    `;
        ...
        document.getElementById('component-generate').onclick = () => {
            ...
            const instructions = document.getElementById('component-instructions').value;
            if (!componentType) {
    ...
    function generateComponent(componentType, instructions) {
    ...
    function showComponentPreview(html, explanation) {
    ...
    `;
        ...
        document.getElementById('preview-refine').onclick = () => {
            const refineText = document.getElementById('refine-instructions').value;
            if (!refineText) {
    ...
    function refineComponent(instructions) {
    ...
    function insertComponent() {
        if (!insertionTarget || !currentComponentHtml)
            return;
    ...
    function addStylePanelListeners() {
        ...
        document.getElementById('vis-bg-color')?.addEventListener('change', (e) => {
            applyStyle('background', e.target.value);
            document.getElementById('vis-bg-text').value = e.target.value;
        });
        document.getElementById('vis-bg-text')?.addEventListener('change', (e) => {
            applyStyle('background', e.target.value);
        });
        document.getElementById('vis-text-color')?.addEventListener('change', (e) => {
            applyStyle('color', e.target.value);
            document.getElementById('vis-text-text').value = e.target.value;
        });
        document.getElementById('vis-text-text')?.addEventListener('change', (e) => {
            applyStyle('color', e.target.value);
        });
        document.getElementById('vis-font-size')?.addEventListener('change', (e) => {
            applyStyle('fontSize', e.target.value);
        });
        document.getElementById('vis-padding')?.addEventListener('change', (e) => {
            applyStyle('padding', e.target.value);
        });
        document.getElementById('vis-border-radius')?.addEventListener('change', (e) => {
            applyStyle('borderRadius', e.target.value);
        });
        document.getElementById('vis-shadow')?.addEventListener('change', (e) => {
            applyStyle('boxShadow', e.target.value);
        });
        // Text formatting controls
        document.getElementById('vis-font-family')?.addEventListener('change', (e) => {
            applyStyle('fontFamily', e.target.value);
        });
        document.getElementById('vis-font-weight')?.addEventListener('change', (e) => {
            applyStyle('fontWeight', e.target.value);
        });
        document.getElementById('vis-line-height')?.addEventListener('change', (e) => {
            applyStyle('lineHeight', e.target.value);
        });
        document.getElementById('vis-letter-spacing')?.addEventListener('change', (e) => {
            applyStyle('letterSpacing', e.target.value);
        });
        document.getElementById('vis-text-align')?.addEventListener('change', (e) => {
            applyStyle('textAlign', e.target.value);
        });
        document.getElementById('vis-text-transform')?.addEventListener('change', (e) => {
    ...
    function startResize(e, direction, element) {
        e.preventDefault();
        e.stopPropagation();
        isResizing = true;
        ...
        const onMouseMove = (e) => handleResize(e, element);
        const onMouseUp = () => {
            isResizing = false;
            document.removeEventListener('mousemove', onMouseMove);
            document.removeEventListener('mouseup', onMouseUp);
    ...
    function logVisualChange(type, details) {
        const change = {
            timestamp: Date.now(),
            source: 'visual',
            type,
    ...
    function showQuickNotification(message, color) {
        const notification = document.createElement('div');
    ...
    function showSaveChangesPrompt(summary = '') {
    ...
    `;
        ...
        if (visualChanges.length > 0) {
            ...
            changeList += visualChanges.slice(0, 3).map(c => {
                if (c.type === 'resize')
                    return `• Resized to ${c.details.width} × ${c.details.height}`;
                if (c.type === 'style')
                    return `• Changed ${c.details.property}`;
                if (c.type === 'move')
                    return `• Repositioned element`;
                if (c.type === 'align')
                    return `• Aligned ${c.details.direction}`;
                return `• ${c.type}`;
            }).join('\n');
            if (visualChanges.length > 3)
    ...
    function persistChangesToCode() {
        ...
        if (visualChanges.length > 0) {
            ...
            summary += 'Visual Edits:\n' + visualChanges.map(c => {
                if (c.type === 'resize')
                    return `- Resized ${c.details.selector} to ${c.details.width} × ${c.details.height}`;
                if (c.type === 'style')
                    return `- Changed ${c.details.property} of ${c.details.selector} to ${c.details.value}`;
                if (c.type === 'align')
                    return `- Aligned ${c.details.selector} ${c.details.direction}`;
                if (c.type === 'center')
                    return `- Centered ${c.details.selector}`;
                if (c.type === 'duplicate')
                    return `- Duplicated ${c.details.selector}`;
                if (c.type === 'delete')
                    return `- Deleted ${c.details.selector}`;
                return `- ${c.type}`;
            }).join('\n');
        }
